...
export default function Header() {
  ...
  const navItems = [
    ...
    { id: 'lunapark', label: texts.lunapark, href: '#lunapark' },
  ...
  const motionStyles = `
    ...
    .motion-header-arrival { animation: smoothEntrance 0.8s cubic-bezier(0.22, 1, 0.36, 1) forwards; }
    .motion-dropdown-elastic { animation: elasticPop 0.4s cubic-bezier(0.34, 1.56, 0.64, 1) forwards; }
    .bezier-transition { transition: all 0.4s cubic-bezier(0.25, 1, 0.5, 1); }
  `;
  ...
  return (
    <>
      ...
      {/* header s plavnim perexodom */}
      <header className={`fixed top-0 left-0 w-full z-50 px-4 lg:px-8 py-1 lg:py-1 flex justify-between items-center motion-header-arrival transition-all duration-500 cubic-bezier(0.25, 1, 0.5, 1) ${
        scrolled 
        ...
        {/* loqo — UVELICHENO */}
        <div className="flex items-center select-none transition-transform duration-500 cubic-bezier(0.34, 1.56, 0.64, 1) hover:scale-[1.04] active:scale-[0.97] pl-4 md:pl-6">
          <Image
            src="/loqo-af.png"
            alt="AF Hotel & Resort"
            width={160}
          ...
          {navItems.map((item) => (
            <div key={item.id} className="relative py-2 group">
              <a 
                ...
                className="transition-colors duration-300 cubic-bezier(0.25, 1, 0.5, 1)"
              ...
              <div className={`absolute bottom-0 left-1/2 -translate-x-1/2 h-0.5 bg-[#00b5d5] transition-all duration-300 cubic-bezier(0.25, 1, 0.5, 1) rounded-full ${
        ...
        <div className="flex items-center space-x-4"> 
          ...
          <a href="tel:+994501234567" className="hidden xl:flex items-center space-x-1.5 text-[12.5px] font-extrabold text-[#00b5d5] hover:text-[#007a91] transition-all duration-300 cubic-bezier(0.25, 1, 0.5, 1) hover:scale-[1.02] active:scale-[0.98]">
          ...
          <div className="relative" ref={langRef}>
            <button 
              ...
              className="flex items-center space-x-1.5 font-black text-[11.5px] tracking-wider text-slate-700 outline-none cursor-pointer uppercase p-1.5 hover:opacity-80 transition-all duration-300"
            >
              <FlagIcon code={currentLang} />
              <span>{currentLang}</span>
              <svg className={`w-2.5 h-2.5 transition-transform duration-500 cubic-bezier(0.34, 1.56, 0.64, 1) text-slate-400 ${langOpen ? 'rotate-180 text-[#00b5d5]' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3.5}><path d="M19 9l-7 7-7-7" /></svg>
            </button>
            
            {langOpen && (
              <div className="absolute right-0 mt-2.5 w-24 bg-white border border-stone-200/80 rounded-xl shadow-[0_12px_40px_rgba(0,0,0,0.09)] py-1 z-50 motion-dropdown-elastic">
                {(['az', 'en', 'ru'] as LangType[]).map((lng) => (
                  <button
                    key={lng}
                    onClick={() => { setLang(lng); setLangOpen(false); }}
                    className={`w-full flex items-center space-x-2 px-3 py-2 text-[11px] font-bold tracking-wider text-slate-700 hover:bg-stone-50 transition-colors duration-200 uppercase ${lang === lng ? 'bg-stone-50 text-[#00b5d5]' : ''}`}
                  >
                    <FlagIcon code={lng} />
                    <span>{lng}</span>
                  </button>
          ...
          <a 
            href="#booking" 
            className="hidden sm:flex items-center space-x-1.5 text-[10.5px] font-black uppercase tracking-widest px-4 py-2.5 bg-[#ff6c02] text-white hover:bg-[#e55f00] rounded-xl shadow-md shadow-[#ff6c02]/10 transition-all duration-500 cubic-bezier(0.34, 1.56, 0.64, 1) hover:scale-[1.05] hover:shadow-[0_6px_20px_rgba(255,108,2,0.25)] active:scale-[0.96]"
          >
          ...
          >
            <span className={`block w-6 h-0.5 bg-slate-800 transition-all duration-500 cubic-bezier(0.34, 1.56, 0.64, 1) ${isOpen ? 'rotate-45 translate-y-2' : ''}`} />
      ...
      {/* mobilnoye menu */}
      <div className={`fixed inset-0 z-40 bg-white/99 backdrop-blur-xl flex flex-col justify-center items-center transition-all duration-700 cubic-bezier(0.22, 1, 0.36, 1) lg:hidden ${
        ...
        <nav className="flex flex-col items-center space-y-6 text-center mb-6">
          {navItems.map((item, index) => (
            <a 
              ...
              className={`text-xl font-black tracking-wider transition-all duration-500 cubic-bezier(0.23, 1, 0.32, 1) ${
        ...
        <div 
          ...
          className={`flex flex-col items-center space-y-4 border-t border-stone-200/60 pt-6 w-2/3 text-center transition-all duration-500 ${
            isOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-95'
          }`}
        >
          <a href="tel:+994501234567" className="text-base font-black text-[#00b5d5] active:scale-95 transition-transform duration-200">
            +994 50 123 45 67
          </a>
        </div>
      </div>
