...
//  slojnyy kontent s lokalizatsiyey dlya vsekh yazykov
  const content = {
    ...
    successDesc: { az: 'Menecerimiz rezervasiyanı təsdiqləmək üçün 15 dəqiqə ərzində sizinlə əlaqə saxlayacaк.', en: 'Our booking manager will contact you within 15 minutes to confirm your reservation.', ru: 'Наш менеджер свяжется с вами в течение 15 минут для подтверждения бронирования.' }[currentLang],
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setSuccess(true);
      setFormData({ roomType: '', checkIn: '', checkOut: '', fullName: '', phone: '' });
    } catch (error) {
    ...
    <section id="booking" className="py-32 bg-[#fdfbf7] text-stone-800 scroll-mt-20 relative select-none">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-16 grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
        ...
        <div className="lg:col-span-5 space-y-6 text-left">
          ...
          <h2 className="text-4xl md:text-6xl font-light font-serif tracking-tight text-[#1e325c] leading-[1.15]">
            {content.title}
          </h2>
        ...
        <div className="lg:col-span-7 bg-[#f4f1eb] border border-stone-200 rounded-3xl p-8 md:p-12 shadow-sm">
          {success ? (
            <div className="text-center py-16 space-y-4 animate-fade-in">
              <div className="w-14 h-14 bg-white text-[#00b5d5] rounded-full flex items-center justify-center mx-auto text-xl border border-stone-200 shadow-sm">
                ✓
              </div>
              <h3 className="text-xl font-medium tracking-tight text-[#1e325c]">
              ...
              <button
                ...
                className="mt-4 text-xs uppercase tracking-widest text-[#ff6c02] font-semibold hover:underline cursor-pointer"
              >
                {content.newRequest}
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
             ...
             {/* viber nomera i daty s avtomaticheskim udaleniem nelegalnix simvolov */}
              <div className="space-y-2 text-left">
                ...
                <div className="relative">
                  <select 
                    ...
                    className="w-full bg-white border border-stone-200 text-stone-800 rounded-xl p-4.5 text-sm font-light outline-none focus:border-[#00b5d5] transition-all cursor-pointer appearance-none shadow-sm"
                  >
                  ...
                  <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-4 text-stone-400 text-[9px]">
                    ▼
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
                <div className="space-y-2">
                  <label className="block text-[10px] uppercase tracking-widest text-stone-400 font-bold">
                    {content.nameLabel} <span className="text-red-500 ml-0.5 text-xs">*</span>
                  </label>
                  <input 
                    name="fullName"
                    required 
                    ...
                    onChange={handleInputChange}
                    className="w-full bg-white border border-stone-200 text-stone-900 placeholder-stone-300 rounded-xl p-4.5 text-sm font-light outline-none focus:border-[#00b5d5] transition-all cursor-pointer shadow-sm" 
                  />
                </div>
                <div className="space-y-2">
                  <label className="block text-[10px] uppercase tracking-widest text-stone-400 font-bold">
                    {content.phoneLabel} <span className="text-red-500 ml-0.5 text-xs">*</span>
                  </label>
                  <input 
                    name="phone"
                    required 
              ...
              <button 
                type="submit" 
                disabled={loading}
                className="w-full bg-[#ff6c02] hover:bg-[#e55f00] text-white font-semibold text-xs uppercase tracking-widest py-4.5 rounded-xl transition-all duration-300 cursor-pointer active:scale-[0.99] disabled:opacity-60 disabled:cursor-not-allowed shadow-md hover:shadow-lg"
              >
                {loading ? (
